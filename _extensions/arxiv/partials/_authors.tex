...
$-- %%%%%%%%%%%%%%%
\textbf{$it.name.literal$}$if(it.orcid)$~\orcidlink{$it.orcid$}$endif$$if(it.attributes.corresponding)$\textsuperscript{*}$endif$$if(it.attributes.equal-contributor)$\textsuperscript{\dag}$endif$$for(it.affiliations)$\\$it.department$\\$it.name$\\$if(it.city)$$it.city$$if(it.postal-code)$,\ $it.postal-code$$endif$$endif$$endfor$\\$if(it.email)$\href{mailto:$it.email$}{$it.email$}$endif$
